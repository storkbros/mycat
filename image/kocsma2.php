  ...
  <div class="row">
	...
	?>
	 ...
	 <div class="col" style="min-width: 300px;max-width: 310px;">
	  <ul class="price<?php if($aktiv == 1 and $block <> $aktivq ){ echo 1; } ?>"> 
		...
		<li style="padding:5px"><img src="image/quest/<?php echo $row["link"]; ?>" class="img-responsive" alt="quest" width="100%" height="250px"> </li>
		<?php 
		$itemid = $row["itemid"];
		$sql = "SELECT * FROM kaja WHERE tipus ='3' AND id='$itemid';";
			$result = $conn->query($sql);
		...
		<li style="padding:5px"><?php echo $row["name"]; ?><img src="image/Food/<?php echo $row["link"]; ?>" class="img-responsive" alt="quest" width="40px" height="40px"> </li>
	...
	?>
	 ...
	 <div class="col" style="min-width: 300px;max-width: 310px;">
	  <ul class="price<?php if($aktiv == 1 and $block <> $aktivq ){ echo 1; } ?>"> 
		...
		<?php if( $aktiv == 1 and $aktivq == $block ) { ?> <li li id="teszt" >
		<progress value="0" max="<?php echo $idokemax; ?>" id="progressBar" style="visibility: visible;"></progress></li>
		<?php } ?> 
		<li class="grey">
		...
		<?php if ( $aktivq == $block ) { 
		?> <div id="mydivtag"></div> <?php 
		} else { if ( $myenergy < $row["energy"] ) { ?><button type="button" class="btn btn-danger" style="font-weight: bold;color:white;" disabled><i class='fas fa-battery-half' style='font-size:20px;color:white;vertical-align:-2px;'></i> <?php echo ($myenergy-$row["energy"]); ?> </button>       <?php }
		if ( $aktiv == 0 and $myenergy >= $row["energy"] ) {?> 
